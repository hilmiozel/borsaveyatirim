...
for hisse_kodu in tum_hisseler:
    ...
    if finansaloranlartahmin_table:
        ...
        for row in rows[1:]:  # İlk satır başlıkları içerdiğinden atlanır
            ...
            fk = float(columns[1].text.strip().replace('.', '').replace(',', '.')) if columns[1].text.strip().isdigit() else 0.0
            fdfavök = float(columns[2].text.strip().replace('.', '').replace(',', '.')) if columns[2].text.strip().isdigit() else 0.0
            fdsatis = float(columns[3].text.strip().replace('.', '').replace(',', '.')) if columns[3].text.strip().isdigit() else 0.0
            pddd = float(columns[4].text.strip().replace('.', '').replace(',', '.')) if columns[4].text.strip().isdigit() else 0.0
    ...
    if yurtdisiiskonto_table:
        ...
        for row in rows[2:]:  # İlk satır başlıkları içerdiğinden atlanır
            columns = row.find_all('td')
            kod = columns[0].text.strip()  # Kod sütunu
            son_fiyat = float(columns[1].text.strip().replace('.', '').replace(',', '.')) if columns[1].text.strip().isdigit() else 0.0
            fk = float(columns[2].text.strip().replace('.', '').replace(',', '.')) if columns[2].text.strip().isdigit() else 0.0
            yurtdisi_fk_prim_oran = float(columns[3].text.strip().replace('.', '').replace(',', '.')) if columns[3].text.strip().isdigit() else 0.0
    ...
    if tarihselortalamalar_table:
        ...
        for row in rows[2:]:  # İlk satır başlıkları içerdiğinden atlanır
            ...
            kod = columns[0].text.strip()  # Kod sütunu
            fk = float(columns[1].text.strip().replace('.', '').replace(',', '.')) if columns[1].text.strip().isdigit() else 0.0
            tarihsel_fk_prim_oran = float(columns[2].text.strip().replace('.', '').replace(',', '.')) if columns[2].text.strip().isdigit() else 0.0
